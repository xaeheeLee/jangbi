# 포크레인 배차 매칭 앱 — 최종 개발 계획서 (v2.1)

> 본 문서는 v2.0 통합 계획서에 추가 검토 결과(P0/P1/P2 패치)를 모두 반영한 단일 계획서입니다.
> P0 항목은 본문에 직접 통합되었고, P1/P2 항목은 섹션 12·13에 정리되어 있습니다.
...
12. [P1 권장 개선 (운영 직후 적용)](#12-p1-권장-개선-운영-직후-적용)
13. [P2 향후 개선 (운영 안정 후 적용)](#13-p2-향후-개선-운영-안정-후-적용)
14. [락 순서 원칙 (개발 가이드)](#14-락-순서-원칙-개발-가이드)
15. [마이그레이션 적용 체크리스트](#15-마이그레이션-적용-체크리스트)
...
| work_date | timestamptz NOT NULL | 작업 시작 일시 |
...
| memo | text | 메모 |
| status | text DEFAULT 'open' (확장 enum, 아래 참조) | |
...
- `matched` — 매칭 완료
- `completed` — 작업 완료 (작업일 24시간 후 자동 전환)
- `cancelled_by_poster` — 등록자 취소 (P1에서 추가)
- `cancelled_by_worker` — 매칭된 기사 취소 (P1에서 추가)
- `expired` — 시간 경과 자동 마감
...
| type | text CHECK ('new_job', 'match_success', 'match_fail', 'priority_expired', 'schedule_conflict', 'job_cancelled') | |
| title | text NOT NULL | |
| body | text | |
...
#### 2.5.3 우선 지원 RPC (트랜잭션 보장 + 충돌 체크 통합)
...
CREATE OR REPLACE FUNCTION apply_with_priority(
  ...
  p_force_apply boolean DEFAULT false  -- 일정 충돌 무시하고 진행
)
RETURNS jsonb AS $$
DECLARE
  v_ticket_id uuid;
...
#### 2.5.5 우선 배차 마감 처리 RPC (60초 후 자동 채택, 만료일 연장 포함)
...
-- 2) 매일 자정: 지난 일감 자동 마감 (open/priority_window만)
SELECT cron.schedule(
  'expire-old-jobs',
  '0 0 * * *',
  $$
...
-- 3) 매일 새벽 1시: matched → completed 자동 전환
SELECT cron.schedule(
  'auto-complete-jobs',
  '0 1 * * *',
  $$
...
- 카카오맵 WebView 위젯 (핀 표시)
- 작업 정보, 장비 조건, 메모
- 하단 CTA: "지원하기" / "우선 지원" (배차권 보유 시)
- 우선 배차 진행 중이면 카운트다운 타이머 표시
- 일정 충돌 시 경고 배너 표시
- **본인 등록 일감일 때:** 매칭 전이면 "취소하기" 버튼 (P1-1)

**일 등록 (`/jobs/create`)**

- Form: 날짜/시간 피커, 지역명 입력
- 카카오맵 WebView: 핀 드래그 or 카카오 로컬 API 주소 검색 (Geocoding)
- 장비 조건 드롭다운
- 등록 완료 → "우선 배차권 +1" 토스트
...
myApplicationsProvider     ← 내 지원 내역 (P1-3)
...
3. 우선 배차권 보유 기사가 "우선 지원" 클릭
   ...
   → 트랜잭션 내에서: 충돌 체크 → 배차권 차감 → 점수 계산 → 지원 등록
   → 충돌 시 SCHEDULE_CONFLICT → 다이얼로그 → p_force_apply=true로 재호출
...
**충돌 정의 (v2.1):** 같은 날짜에 2건 이상의 배차(matched/completed) 일정이 있는 경우.
**P1-2 반영 시:** 시간대 겹침 기반으로 변경 예정 (섹션 12 참조).

**적용 시점:**

1. **캘린더 화면:** 해당 날짜에 ⚠️ 경고 아이콘
2. **일감 지원 시:** `apply_with_priority` 또는 `apply_general` RPC 내부에서 충돌 체크 → `SCHEDULE_CONFLICT` 에러 → 확인 다이얼로그 → "그래도 진행" 시 `p_force_apply: true`로 재호출
...
| 일감 취소 | cancel_job RPC | 매칭된 기사 (있을 시) | High |
...
| NOT_AUTHORIZED | 권한 없음 (P1) | "이 작업을 수행할 권한이 없습니다" |
| CANNOT_MODIFY_MATCHED_JOB | 매칭 후 수정 불가 (P1) | "매칭된 일감의 핵심 정보는 수정할 수 없습니다" |
| OTP_NOT_VERIFIED | OTP 인증 안됨 (P2) | "OTP 인증이 필요합니다" |
| PHONE_ALREADY_EXISTS | 전화번호 중복 (P2) | "이미 사용 중인 전화번호입니다" |

### 7.2 Flutter 에러 핸들링

```dart
Future<void> applyToJob(String jobId, bool isPriority, {bool force = false}) async {
  try {
    ...
    switch (e.message) {
      case 'SCHEDULE_CONFLICT':
        if (!force) {
          ...
          if (proceed) {
            await applyToJob(jobId, isPriority, force: true);
          }
        }
      case 'SELF_APPLY':
...
|-------|------|------------|---------|
| Phase 1 | 3주 (1~3주차) | 인증 + 프로필 + 앱 골격 | M1 |
| Phase 2 | 3주 (4~6주차) | 일감 CRUD + 지도 | M2 |
| Phase 3 | 4주 (7~10주차) | 매칭 엔진 (우선/일반) | M3 |
| Phase 4 | 2주 (11~12주차) | 푸시 알림 | M4 |
| Phase 5 | 3주 (13~15주차) | 캘린더 + 일정 충돌 | M5 |
| Phase 6 | 3주 (16~18주차) | 폴리싱 + QA + 배포 준비 | M6 |
| **총 기간** | **약 18주 (4.5개월)** | | |
...
| 2-3 | 일 등록 화면 (카카오맵 WebView 핀 + 카카오 로컬 API Geocoding) | 좌표/주소 저장 확인 |
| 2-4 | 일감 목록 화면 (필터/정렬) | 장비 필터, 거리순/날짜순 |
| 2-5 | 일감 상세 화면 (카카오맵 핀 + 정보) | 상세 정보 표시 |
...
| 3-3 | apply_with_priority RPC 배포 + 테스트 (충돌 체크 포함) | 트랜잭션 원자성 검증 |
| 3-4 | apply_general RPC 배포 + 테스트 | 선착순 + 장비 검증 + 일정 충돌 체크 |
| 3-5 | finalize_priority_match RPC 배포 (만료일 연장 포함) | FOR UPDATE SKIP LOCKED 검증 |
| 3-6 | Edge Function: process-priority-match 배포 | 60초 후 자동 채택 |
| 3-7 | pg_cron 설정 (priority-windows, expire-old-jobs, auto-complete) | 스케줄 실행 확인 |
| 3-8 | 지원 UI (우선/일반 분기 + 카운트다운 타이머 + 충돌 다이얼로그) | 버튼 상태 분기 |
...
| 3-12 | 탈락자 배차권 반환 검증 (만료일 연장 포함) | 반환 후 7일+ 유효 |

**✅ 마일스톤 M3:** 우선 지원 60초 → 자동 채택 → 상대방 정보 열람 + 일반 선착순 매칭

#### Phase 4 — 푸시 알림 (11~12주차)
...
- **해결:** 모든 핵심 로직을 SECURITY DEFINER RPC 함수에 통합. `FOR UPDATE SKIP LOCKED` 비관적 락. `UNIQUE(job_id, applicant_id)` 제약. 락 순서 표준화 (섹션 14 참조).
...
- **해결:** `idx_jobs_worker_date` 복합 인덱스. 월 단위 페이지네이션. `auto-complete-jobs` cron으로 상태 정리.
...
│   │   │   └── widgets/         # JobCard, FilterChips, CountdownTimer, CancelButton
│   │   ├── providers/
│   │   ├── data/
│   │   └── domain/              # Job 모델
│   ├── dispatch/
...
│   │   ├── presentation/
│   │   └── providers/
│   ├── applications/            # P1-3: 내 지원 내역
│   │   ├── presentation/
│   │   └── providers/
│   └── profile/
│       ├── presentation/        # profile_edit, posted_jobs, settings, phone_change
│       └── providers/
├── shared/
...
│   ├── 007_views.sql
│   ├── 008_functions.sql           # P0 패치 통합 (apply_with_priority, finalize 만료일 연장)
│   ├── 009_triggers.sql
│   ├── 010_indexes.sql
│   ├── 011_rls.sql                 # P1-3 정책 통합
│   ├── 012_cron.sql                # P0-3 auto-complete cron 통합
│   ├── 013_job_lifecycle.sql       # P1-1 (cancel_job, status enum 확장)
│   ├── 014_time_overlap.sql        # P1-2 (work_end_date)
│   ├── 015_phone_change.sql        # P2-1
│   └── 016_notification_cleanup.sql # P2-2
├── functions/
...
## 12. P1 권장 개선 (운영 직후 적용)

### 12.1 일감 취소/수정 정책 (P1-1)

**적용 시점:** 출시 직후, 사용자가 실제 일감을 등록하기 시작할 때.
...
ALTER TABLE jobs ADD CONSTRAINT jobs_status_check CHECK (
  status IN (
    'open', 'priority_window', 'matched', 'completed',
...
BEGIN
  ...
  -- 매칭 전 취소: 등록자 배차권 즉시 만료 + 우선 지원자 배차권 반환
  ...
  UPDATE jobs SET status = 'cancelled_by_poster' WHERE id = p_job_id;
...
**Flutter 변경:**
- `/jobs/detail/:id`에 본인 등록 일감일 때 "취소하기" 버튼 추가
- 매칭 후 시간/위치 등 핵심 필드 수정 UI 비활성화
- `cancel_job` 호출 시 매칭된 기사가 있으면 알림 발송

### 12.2 시간대 기반 충돌 체크 (P1-2)

**적용 시점:** 사용자가 같은 날 다른 시간 작업이 가능하다는 피드백을 줄 때.

**SQL 변경:**

```sql
-- ① 작업 종료 시간 컬럼 추가
ALTER TABLE jobs ADD COLUMN work_end_date timestamptz;
...
-- ② 충돌 체크 로직 변경 (apply_with_priority, apply_general 둘 다)
-- 기존:
--   AND work_date::date = v_job.work_date::date
-- 신규:
--   AND tstzrange(work_date, COALESCE(work_end_date, work_date + interval '4 hours'))
--       && tstzrange(v_job.work_date, COALESCE(v_job.work_end_date, v_job.work_date + interval '4 hours'))
```

**Flutter 변경:**
- 일감 등록 화면에 "예상 작업 시간" 드롭다운 추가 (2시간 / 4시간 / 8시간 / 직접 입력)
- 캘린더 충돌 표시도 시간대 겹침 기반으로 변경

### 12.3 지원 이력 RLS 정책 (P1-3)

**적용 시점:** "내 지원 내역" 화면 추가 시점.

이미 섹션 2.4 RLS 정책에 통합되어 있습니다. Flutter 측에서 화면만 추가하면 됩니다:
...
`/my/applications` 라우트 추가, 마이페이지 메뉴에 진입점 추가.

---

## 13. P2 향후 개선 (운영 안정 후 적용)

### 13.1 전화번호 변경 RPC (P2-1)

**적용 시점:** 사용자가 번호 변경 요청을 보낼 때.
...
BEGIN
  ...
  UPDATE profiles SET phone = p_new_phone WHERE id = p_user_id;
  RETURN jsonb_build_object('status', 'changed', 'new_phone', p_new_phone);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
```

**Flutter 흐름:** 마이페이지 → 전화번호 변경 → 새 번호 입력 → OTP 발송 → 인증 성공 → `change_phone_number(p_otp_verified: true)` 호출

### 13.2 알림 페이지네이션 + 자동 정리 (P2-2)

**적용 시점:** 알림 누적으로 성능 저하가 보일 때.
...
**Flutter 변경:**
- `/my/notifications` 화면을 `ScrollController` 기반 페이지네이션 (`PAGE_SIZE = 30`)
...
### 출시 전 (Phase 1~6 진행 중 기본 적용)

- [ ] 001~012 마이그레이션 적용 (P0 패치 모두 본문 통합됨)
- [ ] cron 4개 등록 확인 (`SELECT * FROM cron.job`)
  - process-priority-windows
  - expire-old-jobs
  - auto-complete-jobs
  - notify-expiring-tickets
- [ ] 동시 지원 시나리오 테스트 (이중 매칭 없음 확인)
- [ ] 탈락자 배차권 만료일 연장 검증 (`expires_at >= now() + 7 days`)

### 출시 직후 (P1 적용)

- [ ] `013_job_lifecycle.sql` 작성 및 적용
- [ ] `014_time_overlap.sql` 작성 및 적용
- [ ] Flutter: 일감 취소 UI, 작업 시간 입력, 내 지원 내역 화면

### 운영 안정 후 (P2 적용)

- [ ] `015_phone_change.sql` 작성 및 적용
- [ ] `016_notification_cleanup.sql` 작성 및 적용
- [ ] Flutter: 전화번호 변경 흐름, 알림 페이지네이션
...
|------|------|------|
| v1.0 | 2026.03 | 최초 작성 |
| v2.0 | 2026.04 | 아키텍처 검토 결과 통합 (7개 치명 버그 수정 + 5개 설계 개선) |
| v2.1 | 2026.05 | 추가 검토 P0 패치 본문 통합, P1/P2 섹션 추가, 18주 로드맵 |
